...
def main():
    ...
    # model_test = torchvision.models.vgg16(num_classes=11).cuda()
    model_test = mobile_vit_small(num_classes=11).cuda()
    # model_test = GoogLeNet(num_classes=10).cuda()
    # model_test.load_state_dict(torch.load(Path('train_fgsm_output/resnet18_ll.pth')))0/81
    model_test.load_state_dict(torch.load(Path('train_fgsm_output/mobile_vit_small_trapll_fgsm.pth')))
    # model_test.load_state_dict(torch.load(Path('train_fgsm_output/vgg16_fgsm_lc.pth'))) 0/70
    # model_test.load_state_dict(torch.load(Path('train_fgsm_output/GoogLeNet_fgsm_cc.pth'))) 0/92
    model_test.float()
    model_test.eval()
