...
def get_args():
    ...
    parser.add_argument('--fname', default='./eval/117.pth', type=str) #0.45smoothing的最终模型
    ...
    parser.add_argument('--attack', default='torchattack', type=str, choices=['pgd', 'fgsm', 'cw','aa','none','torchattack'])
    ...
    if args.attack == 'none':
        ...
        with torch.no_grad():
            ...
            logger.info('Clean Loss: %.4f, Clean_Acc: %.4f -- Test Loss: %.4f, Before_Acc: %.4f,Acc: %.4f',
                        ...
                        total_acc / n)
    elif args.attack == 'torchattack':
        # pretrained_generator_path = './models_adv/8-3netG_epoch_60.pth'
        # pretrained_G = models_adv.Generator(1, 1).cuda()
        ...
        for i, (x,y) in enumerate(test_loader):
            ...
            # atk = torchattacks.AutoAttack(model,eps=0.3,n_classes=11,seed=0)
            atk = torchattacks.LGV(model,train_loader,eps=0.6, alpha=0.02, steps=50, verbose=True)
            # atk = torchattacks.PGD(model,eps=args.epsilon,alpha=args.alpha,steps=40)
        ...
        for i, (X, y) in enumerate(test_loader):
            ...
            if args.attack == 'pgd':
                # atk = torchattacks.PGD(model,eps=0.6)
                # X_adv = atk(X,y)
