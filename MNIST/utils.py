...
def makeRandom(channel,data,max,min,mean,std,Epoch,epoch,delta):
    # seed = random.randint(1,2)
    seed = 1
    # schedule = lambda t: np.interp([t], [0, Epoch * 0.7, Epoch], [(max-min)/2, 2*max, 5*max])[0]
    # schedule = lambda t: np.interp([t], [0, epoch * 0.5, epoch], [0.25, 1, 10])[0] #toy_net
    schedule = lambda t: np.interp([t], [0, Epoch * 0.5, Epoch], [0.25, 1, 10])[0]  # toy_net
    # schedule = lambda t: np.interp([t], [0, epoch * 0.5, epoch], [0.25, 0.8, 1])[0]  #


    # schedule = lambda t: np.interp([t], [0, epoch * 0.5, epoch], [0.35, 0.8, 1])[0]
    # schedule = lambda t: np.interp([t], [0, epoch * 0.5, epoch], [0.25, 1, 5])[0]
    # a = torch.sign(torch.randn_like(data)) * schedule(epoch)
    if seed ==1:
        a = torch.sign(torch.randn_like(data)) * schedule(epoch)
    else:
        a = torch.sign(delta-0.5)*schedule(epoch)
    # print(epoch)
    # print(a)
    # plt.figure()
    # # X = X + delta
    # plt.imshow(a[1].cpu().squeeze(), cmap="gray")
    # plt.axis('off')
    # plt.savefig('padding_noisy.png', bbox_inches='tight', dpi=300, pad_inches=0.0)
    ...
    # plt.figure()
    # # X = X + delta
    # plt.imshow(data[1].cpu().squeeze(), cmap="gray")
    # plt.axis('off')
    # plt.savefig('padding_init.png', bbox_inches='tight', dpi=300, pad_inches=0.0)
    #
    # plt.figure()
    # # X = X + delta
    # plt.imshow(data_padding[1].cpu().squeeze(), cmap="gray")
    # plt.axis('off')
    # plt.savefig('padding.png', bbox_inches='tight', dpi=300, pad_inches=0.0)
    return data_padding
...
class LabelSmoothingLoss(nn.Module):
    ...
    def forward(self, pred, target):
        index_target = torch.where(target <  self.cls_target)
        index_trap = torch.where(target >= self.cls_target)
        pred = pred.log_softmax(dim=self.dim)
        distribution =  self.smoothing/ (self.cls_target-1)
        ...
        with torch.no_grad():
            ...
            # 框架建立
            true_dist = torch.zeros((pred.size()[0],self.cls_target))
            add_dist = torch.zeros((true_dist.size()[0],self.cls-self.cls_target))
            # 目标填充
            ...
            add_dist[index_target[0],:]=(self.smoothing/(self.cls - self.cls_target))
