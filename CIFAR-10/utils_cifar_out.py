...
def attack_pgd(model, X, y, epsilon, alpha, attack_iters, restarts, opt=None):
    ...
    return max_delta
lower = torch.tensor([0]).to(device)
upper =torch.tensor([1]).to(device)
def attack_pgd_fgsm(model, X, y, epsilon, alpha, attack_iters, restarts, opt=None):
    ...
    for zz in range(restarts):
        ...
        for _ in range(attack_iters):
            ...
            d = delta[index[0], :, :, :]
            g = grad[index[0], :, :, :]
            t_e = torch.tensor([epsilon]).to(device)
    ...
    for i, (X, y) in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            if squeezing:
                ...
                X_defense = defense(X + pgd_delta).to(device)
...
def evaluate_pgd(test_loader, model, attack_iters, restarts,epsilon,device):
    ...
    with torch.no_grad():
        for i, (X, y) in enumerate(test_loader):
            ...
            if squeezing:
                ...
                X_defense = defense(X).to(device)

                output_defense = model(X_defense)
                test_acc_defense += (output_defense.max(1)[1] == y).sum().item()
    return test_loss/n, test_acc/n,test_acc_defense/n

def makeRandom(channel,data,Epoch,epoch):
    ...
    schedule = lambda t: np.interp([t], [0, Epoch * 0.5, Epoch], [0.5, 0.8, 1])[0]
    # schedule = lambda t: np.interp([t], [0, Epoch * 0.5, Epoch], [0.3, 0.5, 1])[0]
    # schedule = lambda t: np.interp([t], [0, epoch * 0.5, epoch], [0.3, 0.5, 1])[0]

    a = torch.sign(torch.randn_like(data)) * schedule(epoch)
    # a = torch.sign(data) * schedule(epoch)
    a = a.to(device)
    data_padding = data + a
    return data_padding
    ...
    # try:
    #     if seed ==3:
    #         r1 = torch.randint(-sys.maxsize, -10, size).to(device)
    #         r2 = torch.sign(torch.randn_like(data)).to(device)
    #         data = data + r1*r2
    #     for i in range(channel):
    #         if seed == 1:
    #             r1 = torch.normal(mean[i],std[i],size).to(device)
    #             data[:, i, :, :] = data[:,i,:,:]+r1[:,i,:,:]
    #             data = torch.clamp(data, min, max)
    #         elif seed ==2:
    #             r1 = torch.normal(schedule(epoch),std[i],size).to(device)
    #             r2 = torch.sign(torch.randn_like(data[:,i,:,:])).to(device)
